...
class goto(Scene):
    ...
    def draw(self):
        ...
        show('girl happy')
        text("What's your name?", pos=(20, 450), backyoffset=0)
        self.input.draw()
...
class movie_end_scene(Scene):
    ...
    def mouse_events(self, event):
        if self.button1.events(event):
            self.next_scene = start_scene()
        elif self.button2.events(event):
            quit()
        else:
    ...
    def draw(self):
        ...
        self.button1 = textbutton("Yes")
        self.button2 = textbutton("No", (400, 330))
        say(g)
